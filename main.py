...
def main():
    ...
    for problem in problems:
        ...
        for input_type in ["symbolic", "verbal"]:
            ...
            # 选择特定层的注意力矩阵
            layer_attn = attentions[target_layer]  # Shape: (num_heads, seq_len, seq_len)
            
            # 4.1 可视化单个注意力头
            head = heads_to_plot[0]
            attn_matrix = layer_attn[head].numpy()
            title = f"Problem {pid} - {input_type.capitalize()} Input\nLayer {target_layer+1}, Head {head}"
            plot_attention_heatmap(attn_matrix, tokens, title)
            
            # 4.2 可视化多个注意力头
            plot_multiple_attention_heads(attn_matrices=attentions, tokens=tokens, layer=target_layer, heads=heads_to_plot, problem_id=pid, input_type=input_type)
            
            # 4.3 可视化平均注意力
            plot_average_attention_heatmap(attn_matrices=attentions, tokens=tokens, layer=target_layer, problem_id=pid, input_type=input_type)
            ...
            # 输出分析结果
            print(f"Problem {pid} - {input_type.capitalize()} Input:")
            print(f"  Attention Entropy (Layer {target_layer+1}): {analysis_result['attention_entropy']:.4f}")
            print(f"  Keyword Attention Ratio (Layer {target_layer+1}): {analysis_result['keyword_attention_ratio']:.4f}\n")
            ...
            with open(f"results/problem_{pid}_{input_type}_layer_{target_layer+1}_analysis.pkl", "wb") as f:
